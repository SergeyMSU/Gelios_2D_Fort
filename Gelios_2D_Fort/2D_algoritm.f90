module Algoritm
    ...
    subroutine Gas_dynamic_algoritm2(SS)
        ...
        if(par_Hydro) call Read_setka_bin(gl_S3, "RR003")   ! ДЛЯ ВОДОРОДА   DD020    K0010
        print*, "B"
        ...
        call Read_setka_bin(SS, "DR002")      ! ОСНОВНАЯ СЕТКА  CC021       V0004 - до перестройки     V0005
        print*, "F"
        call Geo_Set_sxem(SS)
        print*, "G"
    ...
    subroutine MK_algoritm(SS)
        ...
        call Read_setka_bin(gl_S4, "RR002")   ! ДЛЯ ВОДОРОДА (Предыдущий расчёт)

        ! call Read_setka_bin(SS, "CC035")      ! ОСНОВНАЯ СЕТКА
        ! call Read_setka_bin(SS, "AA001")      ! ОСНОВНАЯ СЕТКА
        call Read_setka_bin(SS, "DR002")      ! ОСНОВНАЯ СЕТКА
